(() => {
'use strict';
const STORAGE_KEY='commandCenter.v2.7.cloud';
const LEGACY_STORAGE_KEYS=['commandCenter.v2.6.cloud','banqiaoCommandCenter.v2.5.cloud','banqiaoCommandCenter.v2.2','banqiaoCommandCenter.v2.1','banqiaoCommandCenter.m1.snapshots.v1'];
const metrics=[
...
function connectCloud(user){
  ...
  cloud.unsubscribe=cloud.doc.onSnapshot(async snap=>{
    ...
    try{
      ...
      metrics.forEach(m=>{if(state.storeTargets[m.key]===undefined)state.storeTargets[m.key]=0});state.people.forEach(p=>ensurePerson(p.id,p.name));ensureOpsState();ensureBranding();rebuildLineActualsFromSnapshots();
...
function saveBranding(){
  ...
  save();renderBranding();alert('基本設定已儲存，首頁與報表標題已同步更新。');
}
    ...
    dataRows.slice(1).forEach(r=>{
      ...
      if(id&&accyGroup==='ACCY'){
        ...
        if(isAirPods)airPodsExcluded+=saleAmount;
          ...
          if(!insuranceSnapshots[dk][id])insuranceSnapshots[dk][id]={name:p.name,phoneInsurance:0,computerInsurance:0,tabletInsurance:0,watchEarInsurance:0,totalInsurance:0,accyNA:0};
...
function renderAll(){renderWeekOptions();renderStore();renderForecast();renderAiAnalysis();renderTodayMinimums();renderFocus();renderTodayTargets();renderPeople();renderWeekly();renderLag();renderMeeting();renderWeeklyReport();renderMeetingHub();renderShareCard();renderGoalCard();renderBranding();if(state.lastImport){$('lastUpdated').textContent=new Date(state.lastImport.time).toLocaleString('zh-TW');$('lastUpdated').className='status-chip ok'}save()}
...
function renderShareCard(){
  ...
  $('sharePeople').innerHTML=todayWorkingPeople().map(p=>{const t=todayTargetsForPerson(p);return `<tr><td><strong>${esc(p.name)}</strong></td>${coreForecast.map(k=>`<td>${fmt(t[k].remaining)}</td>`).join('')}</tr>`}).join('')||'<tr><td colspan="6">今日無銷售人員排班</td></tr>';
}
...
function renderGoalCard(){
  ...
  $('goalPeople').innerHTML=people.map(p=>{const t=todayTargetsForPerson(p);return `<tr><td><strong>${esc(p.name)}</strong></td>${coreForecast.map(k=>`<td>${fmt(t[k].remaining)}</td>`).join('')}</tr>`}).join('')||'<tr><td colspan="6">今日無銷售人員排班</td></tr>';
...
function renderAll(){renderWeekOptions();renderStore();renderForecast();renderAiAnalysis();renderTodayMinimums();renderFocus();renderTodayTargets();renderPeople();renderSchedule();renderWeekly();renderLag();renderMeeting();renderWeeklyReport();renderMeetingHub();renderShareCard();renderGoalCard();renderBranding();if(state.lastImport){$('lastUpdated').textContent=new Date(state.lastImport.time).toLocaleString('zh-TW');$('lastUpdated').className='status-chip ok'}save()}
