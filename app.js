...
async function importWorkbook(file){
  ...
  try{
    ...
    // 門號表：優先讀 I～L 的「當日人員摘要」。
    // 同一天重複匯入會覆蓋該日快照，不會重複加總。
    ...
    lineRows.slice(1).forEach(r=>{
      const personText=String(r[8]??'').trim();
      if(!personText)return;
      const id=personId(personText);if(!id)return;
      const line=num(r[9]),ganp=num(r[10]),above999=num(r[11]);
      // 空白摘要列略過；0 是有效數值。
      if(r[9]===''&&r[10]===''&&r[11]==='')return;
      daily[id]={name:personText,totalLines:line,ganp,above999};
      summaryRows++;
      if(!state.people.find(x=>x.id===id)){ensurePerson(id,personText);unknownPeople.add(personText);}
    });

    let snapshotSource='I～L 當日摘要';
    // 若瀏覽器讀不到 Excel 公式快取，改以 A～E 原始明細中最新日期計算該日。
    if(summaryRows===0){
      let latest=null;
      lineRows.slice(1).forEach(r=>{const d=excelDate(r[0]);if(d&&(!latest||d>latest))latest=d;});
      if(latest){
        const y=latest.getFullYear(),m=latest.getMonth(),day=latest.getDate();
        lineRows.slice(1).forEach(r=>{
          const d=excelDate(r[0]);if(!d||d.getFullYear()!==y||d.getMonth()!==m||d.getDate()!==day)return;
          const id=personId(r[3]);if(!id)return;
          const display=String(r[4]||id),type=String(r[2]||'').trim().toUpperCase(),plan=num(r[1]);
          if(!daily[id])daily[id]={name:display,totalLines:0,ganp:0,above999:0};
          daily[id].totalLines+=1;
          if(type==='GA'||type==='NP')daily[id].ganp+=1;
          if(plan>=999)daily[id].above999+=1;
          if(!state.people.find(x=>x.id===id))ensurePerson(id,display);
        });
        const fallbackDate=`${y}-${String(m+1).padStart(2,'0')}-${String(day).padStart(2,'0')}`;
        if(!state.dailyLineSnapshots[state.monthKey])state.dailyLineSnapshots[state.monthKey]={};
        state.dailyLineSnapshots[state.monthKey][fallbackDate]=daily;
        snapshotSource=`A～E 最新日期 ${fallbackDate}`;
      }
    }else{
      if(!state.dailyLineSnapshots[state.monthKey])state.dailyLineSnapshots[state.monthKey]={};
      state.dailyLineSnapshots[state.monthKey][snapshotDate]=daily;
    }
