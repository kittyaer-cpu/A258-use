(() => {
'use strict';
const STORAGE_KEY='banqiaoCommandCenter.v2.5.cloud';
const LEGACY_STORAGE_KEYS=['banqiaoCommandCenter.v2.2','banqiaoCommandCenter.v2.1','banqiaoCommandCenter.m1.snapshots.v1'];
const metrics=[
...
const state={monthKey:'',people:[],storeTargets:{},dailyLineSnapshots:{},dailyInsuranceSnapshots:{},meetingNotes:{},meetingHubRecords:[],currentMeetingRecordId:null,lastImport:null,lastImportTotals:null,todayDelta:null,diagnostics:null};
...
function renderAll(){renderWeekOptions();renderStore();renderForecast();renderAiAnalysis();renderTodayMinimums();renderFocus();renderTodayTargets();renderPeople();renderWeekly();renderLag();renderMeeting();renderWeeklyReport();renderMeetingHub();renderShareCard();renderGoalCard();if(state.lastImport){$('lastUpdated').textContent=new Date(state.lastImport.time).toLocaleString('zh-TW');$('lastUpdated').className='status-chip ok'}save()}
...
function weekLagRows(week=selectedMeetingWeek(),metricKey=null){
  ...
  return performancePeople().flatMap(p=>trackedMetrics.map(m=>{
    ...
    return{id:p.id,person:p.name,key:k,label:m.label,target,actual,currentRate:rate(target,actual),gap:Math.max(target-actual,0),forecastRate:fr};
...
function renderAll(){renderWeekOptions();renderStore();renderForecast();renderAiAnalysis();renderTodayMinimums();renderFocus();renderTodayTargets();renderPeople();renderSchedule();renderWeekly();renderLag();renderMeeting();renderWeeklyReport();renderMeetingHub();renderShareCard();renderGoalCard();if(state.lastImport){$('lastUpdated').textContent=new Date(state.lastImport.time).toLocaleString('zh-TW');$('lastUpdated').className='status-chip ok'}save()}
...
function ensureOpsState(){
  ...
  if(!state.storeWeeklyTargets)state.storeWeeklyTargets={};
  if(!state.storeWeeklyTargets[state.monthKey])state.storeWeeklyTargets[state.monthKey]=emptyWeekly();
  if(!state.configByMonth[state.monthKey])state.configByMonth[state.monthKey]={workingCodes:['N8','D6','A4'],nonWorkingCodes:['SS','SR','SH','U'],warningThreshold:.02,managerInStoreKpi:true,managerInGoal:false,managerInTracking:false,managerInManpower:true,minDailyTarget:1,reallocateWeekly:true};
  for(let w=1;w<=4;w++)metrics.forEach(m=>{
    const d=state.storeWeeklyTargets[state.monthKey][w][m.key];
    if(typeof d==='number')state.storeWeeklyTargets[state.monthKey][w][m.key]={target:d,actual:0};
    else if(!d)state.storeWeeklyTargets[state.monthKey][w][m.key]={target:0,actual:0};
...
function metricKeyFromTargetHeader(v){const s=String(v||'').replace(/\s/g,'').toUpperCase();if(s.includes('總保險')||s==='保險')return'totalInsurance';if(s.includes('總門號')||s==='門號')return'totalLines';if(s.includes('GA/NP')||s.includes('GA／NP')||s==='GA')return'ganp';if(s.includes('999'))return'above999';return null}
...
function storeTodayRequired(key){const expected=Math.ceil(num(state.storeTargets[key])*num(progressForDay(currentDay()).cumulative));return Math.max(expected-storeActual(key),0)}
function weekOriginalTarget(week,key){const r=weekRanges()[week-1];let wt=0;for(let d=r.start;d<=r.end;d++)wt+=num(progressForDay(d).daily);return Math.round(num(state.storeTargets[key])*wt)}
...
function weekCatchupTarget(week,key){const original=weekOriginalTarget(week,key);if(week<=1)return original;const priorEnd=weekRanges()[week-2].end,priorGap=Math.max(Math.round(num(state.storeTargets[key])*num(progressForDay(priorEnd).cumulative))-cumulativeActualToDay(key,priorEnd),0);return original+(currentConfig().reallocateWeekly?priorGap:0)}
...
function personWeekOriginalTarget(p,week,key){
  ...
  return Math.max(0,Math.round(num(p.data[key]?.target)*weight));
}
...
function personWeekCatchupTarget(p,week,key){
  ...
  const priorExpected=Math.round(num(p.data[key]?.target)*num(progressForDay(priorEnd).cumulative));
...
function renderWeekly(){
  ...
  const storeGrid=storeCard.querySelector('.weekly-metrics');coreForecast.forEach(k=>{const m=metrics.find(x=>x.key===k),original=weekOriginalTarget(week,k),catchup=weekCatchupTarget(week,k),actual=weekActualValue(week,k),r=rate(catchup,actual),box=document.createElement('div');box.className='weekly-metric';box.innerHTML=`<strong>${m.label}</strong><div class="weekly-numbers"><label>公司原定<input value="${original}" disabled></label><label>追趕目標<input value="${catchup}" disabled></label><label>週實績<input value="${actual}" disabled></label></div><span class="rate-pill ${rateClass(r)}">${rateText(r)}</span><small class="weekly-remaining">尚需 ${fmt(Math.max(catchup-actual,0))}</small>`;storeGrid.appendChild(box)});el.appendChild(storeCard);
  performancePeople().forEach(p=>{const card=document.createElement('article');card.className='weekly-card';card.innerHTML=`<div class="weekly-header"><div><strong>${esc(p.name)}</strong><div class="person-id">第 ${week} 週個人目標｜依個人月目標與公司進度</div></div></div><div class="weekly-metrics"></div>`;const grid=card.querySelector('.weekly-metrics');coreForecast.forEach(k=>{const m=metrics.find(x=>x.key===k),original=personWeekOriginalTarget(p,week,k),catchup=personWeekCatchupTarget(p,week,k),actual=personWeekActual(p,week,k),r=rate(catchup,actual),box=document.createElement('div');box.className='weekly-metric';box.innerHTML=`<strong>${m.label}</strong><div class="weekly-numbers"><label>個人原定<input value="${original}" disabled></label><label>個人追趕<input value="${catchup}" disabled></label><label>週實績<input value="${actual}" disabled></label></div><span class="rate-pill ${rateClass(r)}">${rateText(r)}</span><small class="weekly-remaining">尚需 ${fmt(Math.max(catchup-actual,0))}</small>`;grid.appendChild(box)});el.appendChild(card)})
}
...
function weeklyReportText(week){const d=weeklyReportData(week);return [`板橋戰情中心｜第 ${week} 週店長週報`,`期間：${new Date().getMonth()+1}/${d.range.start}～${new Date().getMonth()+1}/${d.range.end}`,'','一、本週 KPI',...d.store.map(x=>`${x.label}：實際 ${fmt(x.actual)}／公司原定 ${fmt(x.original)}／追趕目標 ${fmt(x.target)}（${rateText(x.r)}）`),'','二、本週亮點',...(d.highlights.length?d.highlights.map(x=>`${x.label} 已跟上追趕目標，完成率 ${rateText(x.r)}。`):['本週尚無已完成追趕目標的核心 KPI。']),'','三、落後狀況',...(d.lag.length?d.lag.slice(0,10).map(x=>`${x.person}－${x.label}：月實績 ${fmt(x.actual)}／目標 ${fmt(x.target)}，月底尚差 ${fmt(x.gap)}。`):['本週無個人追蹤項目。']),'','四、店長分析',...d.ai.map(x=>`・${x}`),'','五、下週行動',...performancePeople().map(p=>{const n=getMeetingNote(week,p.id);return n.action?`${p.name}：${n.action}`:''}).filter(Boolean)].join('\n')}
