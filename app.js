(() => {
'use strict';
const STORAGE_KEY='storeKpiManagerM1.v1';
const metrics=[
...
const state={monthKey:'',people:[],storeTargets:{},weekly:{},lastImport:null,diagnostics:null};
...
function load(){try{const old=JSON.parse(localStorage.getItem(STORAGE_KEY)||'null');if(old){Object.assign(state,old);state.monthKey=monthKey();metrics.forEach(m=>{if(state.storeTargets[m.key]===undefined)state.storeTargets[m.key]=0});state.people.forEach(p=>ensurePerson(p.id,p.name))}}catch(e){console.warn(e)}}
...
async function importWorkbook(file){
  ...
  if(typeof XLSX==='undefined'){status.textContent='Excel 解析元件載入失敗，請確認網路後重新整理。';chip.textContent='載入失敗';chip.className='status-chip error';return}
  status.textContent='正在讀取 Excel⋯';chip.textContent='處理中';chip.className='status-chip';
  try{
    const wb=XLSX.read(await file.arrayBuffer(),{type:'array',cellDates:true});
    const required=['Data','對應表','門號表','人員表'];const missing=required.filter(n=>!wb.Sheets[n]);if(missing.length)throw new Error(`缺少工作表：${missing.join('、')}`);
    const dataRows=sheetRows(wb,'Data'),mapRows=sheetRows(wb,'對應表'),lineRows=sheetRows(wb,'門號表'),staffRows=sheetRows(wb,'人員表');
    const previousTargets=new Map(state.people.map(p=>[p.id,{data:p.data,weekly:p.weekly}]));
    state.people=[];
    staffRows.slice(1).forEach(r=>{const id=personId(r[0]);if(!id)return;const label=String(r[2]||`${id}-${r[1]||''}`).trim();const p=ensurePerson(id,label);const old=previousTargets.get(id);if(old){metrics.forEach(m=>p.data[m.key].target=num(old.data?.[m.key]?.target));p.weekly=old.weekly||p.weekly;ensurePerson(id,label)}});
    const skuMap=new Map();mapRows.slice(1).forEach(r=>{const sku=String(r[0]??'').trim().toUpperCase();const key=categoryKey(r[2]);if(sku&&key)skuMap.set(sku,key)});
    const unknownSku=new Map(),unknownPeople=new Set();let dataMatched=0;
    dataRows.slice(1).forEach(r=>{
      const d=excelDate(r[0]);if(!d||monthKey(d)!==state.monthKey)return;
      const sku=String(r[3]??'').trim().toUpperCase(),staffRaw=String(r[6]??'').trim(),id=personId(staffRaw),qty=num(r[7]);if(!sku||!id||!qty)return;
      const key=skuMap.get(sku);if(!key){unknownSku.set(sku,String(r[4]||''));return}
      let p=state.people.find(x=>x.id===id);if(!p){p=ensurePerson(id,staffRaw);unknownPeople.add(staffRaw)}
      p.data[key].actual+=qty;if(key!=='totalInsurance')p.data.totalInsurance.actual+=qty;
      const w=weekOfDate(d);p.weekly[state.monthKey][w][key].actual+=qty;if(key!=='totalInsurance')p.weekly[state.monthKey][w].totalInsurance.actual+=qty;
      dataMatched+=qty;
    });
    // 月門號：依 I～L 欄讀取（I人員、J總門號、K GA/NP、L 999以上）
    lineRows.slice(1).forEach(r=>{const raw=String(r[8]??'').trim();if(!raw)return;const id=personId(raw);let p=state.people.find(x=>x.id===id);if(!p){p=ensurePerson(id,raw);unknownPeople.add(raw)}p.data.totalLines.actual+=num(r[9]);p.data.ganp.actual+=num(r[10]);p.data.above999.actual+=num(r[11])});
    // 每週門號：由門號表 A～D 原始明細按日期重算
    lineRows.slice(1).forEach(r=>{const d=excelDate(r[0]);if(!d||monthKey(d)!==state.monthKey)return;const id=personId(r[3]);if(!id)return;let p=state.people.find(x=>x.id===id);if(!p)p=ensurePerson(id,String(r[4]||id));const w=weekOfDate(d);p.weekly[state.monthKey][w].totalLines.actual+=1;const type=String(r[2]||'').trim().toUpperCase();if(type==='GA'||type==='NP')p.weekly[state.monthKey][w].ganp.actual+=1;if(num(r[1])>=999)p.weekly[state.monthKey][w].above999.actual+=1});
    state.lastImport={file:file.name,time:new Date().toISOString()};state.diagnostics={dataMatched,unknownSku:[...unknownSku],unknownPeople:[...unknownPeople]};
    save();renderAll();
    status.innerHTML=`匯入完成：保險計入 <strong>${fmt(dataMatched)}</strong> 件；門號 <strong>${fmt(storeActual('totalLines'))}</strong>；GA／NP <strong>${fmt(storeActual('ganp'))}</strong>；999以上 <strong>${fmt(storeActual('above999'))}</strong>${unknownSku.size?`。另有 ${unknownSku.size} 個未對應料號未計入。`:''}`;
    chip.textContent=new Date().toLocaleString('zh-TW');chip.className='status-chip ok';
  }catch(err){console.error(err);status.textContent=`匯入失敗：${err.message}`;chip.textContent='匯入失敗';chip.className='status-chip error'}
}
