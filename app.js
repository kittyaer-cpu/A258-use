...
async function importWorkbook(file){
  ...
  try{
    ...
    dataRows.slice(1).forEach(r=>{
      ...
      const sku=String(r[3]??'').trim().toUpperCase(),staffRaw=String(r[6]??'').trim(),id=personId(staffRaw),qty=num(r[7]);
      // 保險達成規則：一般交易與預購交易計入；作廢交易、預售交易、預售結帳及負數數量不計。
      const excludedTransaction = transactionType.includes('作廢') || transactionType.includes('預售');
      if(!sku||!id||qty<=0||excludedTransaction)return;
    ...
    // 門號、GA／NP、999以上：直接由門號表 A～E 原始明細按日期重算。
    // 不依賴 J～M 的 Excel 公式快取，避免手機或網頁匯入時公式尚未重算而顯示 0。
    lineRows.slice(1).forEach(r=>{
      const d=excelDate(r[0]);if(!d||monthKey(d)!==state.monthKey)return;
      const id=personId(r[3]);if(!id)return;
      const displayName=String(r[4]||`${id}`),type=String(r[2]||'').trim().toUpperCase(),plan=num(r[1]);
      let p=state.people.find(x=>x.id===id);if(!p){p=ensurePerson(id,displayName);unknownPeople.add(displayName)}
      const w=weekOfDate(d);
      p.data.totalLines.actual+=1;p.weekly[state.monthKey][w].totalLines.actual+=1;
      if(type==='GA'||type==='NP'){p.data.ganp.actual+=1;p.weekly[state.monthKey][w].ganp.actual+=1}
      if(plan>=999){p.data.above999.actual+=1;p.weekly[state.monthKey][w].above999.actual+=1}
    });
