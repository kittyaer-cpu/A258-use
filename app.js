...
function renderWeekly(){ensureOpsState();const week=Number($('weekSelect').value||weekOfDate(new Date())),el=$('weeklyList');el.innerHTML='';const card=document.createElement('article');card.className='weekly-card';card.innerHTML=`<div class="weekly-header"><div><strong>全店</strong><div class="person-id">第 ${week} 週｜依公司進度表</div></div></div><div class="weekly-metrics"></div>`;const grid=card.querySelector('.weekly-metrics');coreForecast.forEach(k=>{const m=metrics.find(x=>x.key===k),original=weekOriginalTarget(week,k),catchup=weekCatchupTarget(week,k),actual=weekActualValue(week,k),r=rate(catchup,actual),box=document.createElement('div');box.className='weekly-metric';box.innerHTML=`<strong>${m.label}</strong><div class="weekly-numbers"><label>公司原定<input value="${original}" disabled></label><label>追趕目標<input value="${catchup}" disabled></label><label>週實績<input value="${actual}" disabled></label></div><span class="rate-pill ${rateClass(r)}">${rateText(r)}</span><small class="weekly-remaining">尚需 ${fmt(Math.max(catchup-actual,0))}</small>`;grid.appendChild(box)});el.appendChild(card)}
...
function renderMeeting(){
  ...
  const notes=$('meetingNotesList');notes.innerHTML='';const notePeople=lag.length?[...new Map(lag.map(x=>[x.id,{id:x.id,name:x.person}])).values()]:performancePeople();notePeople.forEach(p=>{const n=getMeetingNote(week,p.id),card=document.createElement('article');card.className='meeting-note-card';card.innerHTML=`<div class="meeting-note-head"><strong>${esc(p.name)}</strong><span>第 ${week} 週</span></div><label>本週問題<textarea data-field="issue">${esc(n.issue)}</textarea></label><label>下週改善<textarea data-field="action">${esc(n.action)}</textarea></label><label>下次追蹤<input data-field="followUp" type="date" value="${esc(n.followUp)}"></label>`;card.querySelectorAll('[data-field]').forEach(el=>el.addEventListener('input',e=>{n[e.target.dataset.field]=e.target.value;save()}));notes.appendChild(card)});
  const next=Math.min(week+1,4),nextBody=$('nextWeekBody');nextBody.innerHTML=`<tr><td><strong>全店追趕目標</strong></td>${coreForecast.map(k=>`<td>${fmt(weekCatchupTarget(next,k))}</td>`).join('')}</tr>`
}
