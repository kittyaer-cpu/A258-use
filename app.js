(() => {
'use strict';
const STORAGE_KEY='banqiaoCommandCenter.v2.3.cloud';
...
function weekLagRows(week=selectedMeetingWeek()){
  return performancePeople().flatMap(p=>coreForecast.map(k=>{const target=personWeekCatchupTarget(p,week,k),actual=personWeekActual(p,week,k),monthTarget=num(p.data[k]?.target),f=forecastValue(num(p.data[k]?.actual)),fr=rate(monthTarget,f);if(target<=0||actual>=target)return null;return{id:p.id,person:p.name,key:k,label:metrics.find(m=>m.key===k).label,target,actual,currentRate:rate(target,actual),gap:Math.max(target-actual,0),forecastRate:fr}}).filter(Boolean)).sort((a,b)=>b.gap-a.gap||((a.currentRate??999)-(b.currentRate??999)))
}
function renderMeeting(){
...
function renderMeeting(){
  ...
  const lag=weekLagRows(),peopleBody=$('meetingPeopleBody'),empty=$('meetingPeopleEmpty');peopleBody.innerHTML='';empty.classList.toggle('hidden',lag.length>0);lag.forEach(x=>{const tr=document.createElement('tr');tr.innerHTML=`<td><strong>${esc(x.person)}</strong></td><td>${esc(x.label)}</td><td>${fmt(x.target)}</td><td>${fmt(x.actual)}</td><td><span class="rate-pill ${rateClass(x.currentRate)}">${rateText(x.currentRate)}</span></td><td class="negative">-${fmt(x.gap)}</td><td class="negative">${rateText(x.forecastRate)}</td>`;peopleBody.appendChild(tr)});
