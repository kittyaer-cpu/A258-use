...
function connectCloud(user){
  ...
  cloud.unsubscribe=cloud.doc.onSnapshot(async snap=>{
    ...
    try{
      ...
      metrics.forEach(m=>{if(state.storeTargets[m.key]===undefined)state.storeTargets[m.key]=0});state.people.forEach(p=>ensurePerson(p.id,p.name));ensureOpsState();ensureInspectionState();ensureBranding();ensurePeoplePreferences();rebuildLineActualsFromSnapshots();
...
function markRegularPerson(id,name){const p=ensurePerson(id,name);p.support=false;return p}
function markSupportPerson(id,name){const p=ensurePerson(id,name);if(p.support!==false)p.support=true;return p}
function isSupportPerson(personOrId){const id=String(typeof personOrId==='object'&&personOrId?personOrId.id:personOrId);const p=state.people.find(x=>x.id===id)||(typeof personOrId==='object'?personOrId:null);return !!p?.support}
...
function showPage(page){document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x.dataset.page===page));document.querySelectorAll('.page').forEach(x=>x.classList.remove('active'));const target=$(`page-${page}`);if(target)target.classList.add('active');setMobileNavActive(page==='meeting'?'meeting':page==='report'?'report':page==='inspection'?'inspection':'home')}
...
async function importWorkbook(file){
  ...
  try{
    ...
    if(!state.productMaster||typeof state.productMaster!=='object')state.productMaster={};
    dataRows.slice(1).forEach(r=>{const sku=String(r[3]??'').trim().toUpperCase(),name=String(r[4]??'').trim();if(sku&&name){const old=state.productMaster[sku]||{};state.productMaster[sku]={...old,sku,name,updatedAt:new Date().toISOString()};}});
    ...
    staffRows.slice(1).forEach(r=>{
      ...
      const p=markRegularPerson(id,label);
    ...
    dataRows.slice(1).forEach(r=>{
      ...
      if(id&&accyGroup==='ACCY'){
        ...
        if(!p){p=markSupportPerson(id,staffRaw);unknownPeople.add(staffRaw);}
    ...
    lineRows.slice(1).forEach(r=>{
      ...
      rawRows++;
      if(!state.people.find(x=>x.id===id))markSupportPerson(id,display);
    });
    ...
    if(monthHeader>=0){
      for(let i=monthHeader+1;i<lineRows.length;i++){
        ...
        if(!state.people.find(x=>x.id===id))markSupportPerson(id,personRaw);
      }
    }
...
function renderAll(){renderWeekOptions();renderStore();renderForecast();renderAiAnalysis();renderTodayMinimums();renderFocus();renderTodayTargets();renderPeople();renderWeekly();renderLag();renderMeeting();renderWeeklyReport();renderMeetingHub();renderInspection();renderShareCard();renderGoalCard();renderPersonAliases();renderBranding();if(state.lastImport){$('lastUpdated').textContent=new Date(state.lastImport.time).toLocaleString('zh-TW');$('lastUpdated').className='status-chip ok'}save()}
...
function renderTodayBrief(){
  ...
  if(rows[0])items.push({tone:'warn',icon:'↗',title:`${rows[0].person}｜${rows[0].label}`,value:`差 ${fmt(rows[0].gap)}`,note:`目前 ${rateText(rows[0].currentRate)}，每日需 ${Math.max(1,Math.ceil(num(rows[0].daily)))}`});
...
function renderTodayMinimums(){
  const el=$('todayMinGrid');if(!el)return;el.innerHTML='';dailyGoalKeys().forEach(k=>{const m=metrics.find(x=>x.key===k),v=storeTodayMinimum(k),card=document.createElement('article');card.className='today-min-card';card.innerHTML=`<span>${m.label}</span><strong>+${fmt(v)}</strong><small>今日尚需完成</small>`;el.appendChild(card)})
}
...
function performancePeople(){return state.people.filter(p=>!isManagerId(p.id)&&!isSupportPerson(p))}
...
function bind(){
  ...
  document.querySelectorAll('[data-mobile-action]').forEach(btn=>btn.addEventListener('click',()=>{
    ...
    if(action==='inspection'){showPage('inspection');requestAnimationFrame(()=>window.scrollTo({top:0,behavior:'smooth'}));return}
...
function renderTodayTargets(){
  const body=$('todayTargetBody');body.innerHTML='';const people=todayWorkingPeople(),keys=dailyGoalKeys();
  const table=body.closest('table'),headRow=table?.querySelector('thead tr');
  if(headRow)headRow.innerHTML=`<th>人員</th>${keys.map(k=>`<th>${esc(metrics.find(m=>m.key===k).label)}</th>`).join('')}<th>狀態</th>`;
  if(!people.length){body.innerHTML=`<tr><td colspan="${keys.length+2}">今天沒有可分配目標的銷售人員，或尚未匯入班表。</td></tr>`;return}
  people.forEach(p=>{const t=todayTargetsForPerson(p),status=personTodayStatus(p),tr=document.createElement('tr');tr.innerHTML=`<td><strong>${esc(personDisplay(p))}</strong></td>${keys.map(k=>`<td><span class="today-number">${k==='accyNA'&&todayGoalForPerson(p,k)<=0?'—':fmt(todayGoalForPerson(p,k))}</span><small>今日已完成 ${fmt(t[k].actual)}</small></td>`).join('')}<td><span class="rate-pill ${status.className}">${status.label}</span></td>`;body.appendChild(tr)})
}
function renderShareCard(){
  $('shareDate').textContent=`${new Date().toLocaleDateString('zh-TW')}（${weekdayText()}）`;$('shareWeek').textContent=`第 ${weekOfDate(new Date())} 週`;
  const kpi=$('shareKpiGrid');kpi.innerHTML='';coreForecast.forEach(k=>{const m=metrics.find(x=>x.key===k),a=storeActual(k),t=state.storeTargets[k],r=rate(t,a),div=document.createElement('div');div.className=`share-kpi ${rateClass(r)}`;div.innerHTML=`<span>${m.label}</span><strong>${fmt(a)} / ${fmt(t)}</strong><small>${rateText(r)}</small>`;kpi.appendChild(div)});
  $('shareMinGrid').innerHTML=dailyGoalKeys().map(k=>`<div><span>${metrics.find(x=>x.key===k).label}</span><strong>+${fmt(storeTodayMinimum(k))}</strong></div>`).join('');
...
function weekLagPeopleSummary(week=selectedMeetingWeek()){
  ...
  weekLagRows(week).forEach(x=>{
    if(!grouped.has(x.id))grouped.set(x.id,{id:x.id,person:x.person,items:[],count:0,worstRate:100});
    const row=grouped.get(x.id);row.items.push(x);row.count++;row.worstRate=Math.min(row.worstRate,x.currentRate??100);
  });
  return [...grouped.values()].sort((a,b)=>b.count-a.count||a.worstRate-b.worstRate);
}
...
function renderAll(){renderWeekOptions();renderStore();renderForecast();renderAiAnalysis();renderTodayMinimums();renderFocus();renderTodayTargets();renderPeople();renderSchedule();renderWeekly();renderLag();renderMeeting();renderWeeklyReport();renderMeetingHub();renderInspection();renderShareCard();renderGoalCard();renderPersonAliases();renderBranding();if(state.lastImport){$('lastUpdated').textContent=new Date(state.lastImport.time).toLocaleString('zh-TW');$('lastUpdated').className='status-chip ok'}save()}
...
function metricKeyFromTargetHeader(v){const raw=String(v||'').trim(),s=raw.replace(/\s/g,'').toUpperCase();if(s.includes('手機保險'))return'phoneInsurance';if(s.includes('電腦保險'))return'computerInsurance';if(s.includes('平板保險'))return'tabletInsurance';if(s.includes('手錶/耳機保險')||s.includes('手錶／耳機保險')||s.includes('手錶耳機保險'))return'watchEarInsurance';if(s.includes('總保險')||s==='保險')return'totalInsurance';if(s.includes('總門號')||s==='門號')return'totalLines';if(s.includes('GA/NP')||s.includes('GA／NP')||s==='GA')return'ganp';if(s.includes('999'))return'above999';if(s.includes('ACCY(NA)')||s.includes('ACCYNA')||s==='ACCY')return'accyNA';return null}
...
async function importScheduleWorkbook(file){
  ...
  try{
    ...
    rows.slice(1).forEach(r=>{const id=personId(r[0]);if(!id)return;const name=String(r[1]||id).trim();state.schedulePeople[id]={id,name};markRegularPerson(id,`${id}-${name}`);for(let c=3;c<header.length;c++){const day=Number(header[c]);if(!Number.isFinite(day)||day<1||day>31)continue;const dk=`${state.monthKey}-${String(day).padStart(2,'0')}`;if(!month[dk])month[dk]={};month[dk][id]={code:String(r[c]||'').trim(),name}}});
    ...
    tr.slice(1).forEach(r=>{const label=String(r[0]||'').trim();if(!label)return;if(label.includes('全店')){Object.entries(cols).forEach(([k,i])=>state.storeTargets[k]=num(r[i]));return}const id=personId(label);if(!id)return;const p=markRegularPerson(id,label);Object.entries(cols).forEach(([k,i])=>p.data[k].target=num(r[i]))});
    // 依進度表建立原定週目標
...
function dailyGoalKeys(){
  // 件數型 KPI 每天都保留基本任務；Accy(NA) 全店達成 100% 後自動從今日目標隱藏。
  return coreForecast.filter(key=>{
    if(key!=='accyNA')return true;
    const target=num(state.storeTargets[key]);
    return target>0 && storeActual(key)<target;
  });
}
function todayGoalForPerson(p,key){
  const base=num(todayAllocation(key)[p.id]);
  if(key!=='accyNA')return Math.max(base,1);
  const target=num(p.data[key]?.target),actual=num(p.data[key]?.actual);
  if(target<=0||actual>=target)return 0;
  const remaining=Math.max(target-actual,0),days=Math.max(remainingWorkdays(p.id),1);
  return Math.max(base,Math.ceil(remaining/days));
}
function storeTodayMinimum(key){
  const people=todayWorkingPeople().filter(p=>currentConfig().managerInGoal||!isManagerId(p.id));
  return people.reduce((sum,p)=>sum+todayGoalForPerson(p,key),0);
}
function lagRows(){const rows=[],expected=expectedRate(),remainingDays=Math.max(daysInMonth()-currentDay(),0);performancePeople().forEach(p=>metrics.forEach(m=>{const k=m.key,t=num(p.data[k]?.target),a=num(p.data[k]?.actual);if(t<=0)return;const expectedActual=t*expected/100;if(a+1e-9<expectedActual){rows.push({person:personDisplay(p),id:p.id,key:k,label:m.label,target:t,actual:a,currentRate:rate(t,a),expectedRate:expected,gap:expectedActual-a,daily:remainingWorkdays(p.id)?Math.max(expectedActual-a,0)/remainingWorkdays(p.id):Math.max(expectedActual-a,0),forecast:forecastValue(a),forecastRate:rate(t,forecastValue(a))})}}));return rows.sort((a,b)=>(b.expectedRate-b.currentRate)-(a.expectedRate-a.currentRate)||b.gap-a.gap)}
...
function renderGoalCard(){
  if(!$('goalCard'))return;
  const now=new Date(),keys=dailyGoalKeys();
  $('goalCardDate').textContent=`${now.toLocaleDateString('zh-TW')}（${weekdayText(now)}）`;
  const store=$('goalStoreGrid');
  store.innerHTML=keys.map(k=>`<div><span>${metrics.find(m=>m.key===k).label}</span><strong>${fmt(storeTodayMinimum(k))}</strong></div>`).join('');
  const table=$('goalPeople')?.closest('table'),headRow=table?.querySelector('thead tr');
  if(headRow)headRow.innerHTML=`<th>人員</th>${keys.map(k=>`<th>${esc(metrics.find(m=>m.key===k).label)}</th>`).join('')}`;
  const people=todayWorkingPeople().filter(p=>currentConfig().managerInGoal||!isManagerId(p.id));
  $('goalPeople').innerHTML=people.map(p=>`<tr><td><strong>${esc(personDisplay(p))}</strong></td>${keys.map(k=>{const v=todayGoalForPerson(p,k);return `<td>${k==='accyNA'&&v<=0?'—':fmt(v)}</td>`}).join('')}</tr>`).join('')||`<tr><td colspan="${keys.length+1}">今日無銷售人員排班</td></tr>`;
}
...
let inspectionDraftItems=[];
let displayAuditDraft=null;
let scannerInstance=null;
let scannerTarget='';
function ensureInspectionState(){
  if(!Array.isArray(state.inventoryInspections))state.inventoryInspections=[];
  if(!Array.isArray(state.displayCatalog))state.displayCatalog=[];
  if(!Array.isArray(state.displayAudits))state.displayAudits=[];
  if(!state.productMaster||typeof state.productMaster!=='object')state.productMaster={};
}
...
function productFromCode(code){ensureInspectionState();const key=normalizeBarcode(code);if(!key)return null;if(state.productMaster[key])return state.productMaster[key];return Object.values(state.productMaster).find(x=>normalizeBarcode(x.ean)===key||normalizeBarcode(x.sku)===key)||null}
function rememberProduct(code,name,sku=''){ensureInspectionState();const key=normalizeBarcode(code),realSku=normalizeBarcode(sku)||key;if(!key&&!realSku)return;const existing=state.productMaster[realSku]||{};state.productMaster[realSku]={...existing,sku:realSku,name:String(name||existing.name||'').trim(),ean:key.length===13&&/^\d{13}$/.test(key)?key:(existing.ean||''),updatedAt:new Date().toISOString()};if(key!==realSku)state.productMaster[key]=state.productMaster[realSku]}
function inspectionDifference(systemQty,actualQty){return num(actualQty)-num(systemQty)}
function differenceBadge(diff){const cls=diff===0?'ok':diff>0?'plus':'minus';return `<span class="difference-badge ${cls}">${diff===0?'正常':`${diff>0?'+':''}${fmt(diff)}`}</span>`}
function bindProductLookup(barcodeId,nameId){const input=$(barcodeId),name=$(nameId);if(!input||!name)return;input.addEventListener('change',()=>{const p=productFromCode(input.value);if(p&&!name.value.trim())name.value=p.name||''})}
function switchInspectionTab(tab){document.querySelectorAll('.inspection-tab').forEach(x=>x.classList.toggle('active',x.dataset.inspectionTab===tab));document.querySelectorAll('.inspection-panel').forEach(x=>x.classList.toggle('active',x.id===`inspection-${tab}`))}
function addInspectionItem(){const code=normalizeBarcode($('inspectionBarcode')?.value),name=$('inspectionProductName')?.value.trim()||productFromCode(code)?.name||'',systemQty=num($('inspectionSystemQty')?.value),actualQty=num($('inspectionActualQty')?.value),note=$('inspectionItemNote')?.value.trim()||'';if(!code)return alert('請輸入或掃描料號／EAN。');if($('inspectionSystemQty')?.value===''||$('inspectionActualQty')?.value==='')return alert('請輸入目前庫存與實際庫存。');const existing=inspectionDraftItems.find(x=>x.code===code);if(existing&&!confirm('此品項已在本次抽盤中，是否以目前數量覆蓋？'))return;const item={id:existing?.id||uid('item'),code,sku:productFromCode(code)?.sku||code,name,systemQty,actualQty,difference:inspectionDifference(systemQty,actualQty),note};if(existing)Object.assign(existing,item);else inspectionDraftItems.push(item);if(name)rememberProduct(code,name,item.sku);['inspectionBarcode','inspectionProductName','inspectionSystemQty','inspectionActualQty','inspectionItemNote'].forEach(id=>{if($(id))$(id).value=''});renderInspectionDraft();$('inspectionBarcode')?.focus()}
function renderInspectionDraft(){const el=$('inspectionDraftList');if(!el)return;el.innerHTML='';if(!inspectionDraftItems.length){el.innerHTML='<div class="empty-state">本次尚未加入品項。</div>';return}inspectionDraftItems.forEach((x,i)=>{const row=document.createElement('article');row.className='inspection-draft-item';row.innerHTML=`<div class="inspection-item-index">${i+1}</div><div class="inspection-item-main"><strong>${esc(x.name||x.sku||x.code)}</strong><small>${esc(x.sku||x.code)}${x.code!==x.sku?`｜${esc(x.code)}`:''}</small><span>系統 ${fmt(x.systemQty)}　實際 ${fmt(x.actualQty)}</span>${x.note?`<p>${esc(x.note)}</p>`:''}</div>${differenceBadge(x.difference)}<button type="button" class="icon-delete" aria-label="刪除">×</button>`;row.querySelector('.icon-delete').addEventListener('click',()=>{inspectionDraftItems.splice(i,1);renderInspectionDraft()});el.appendChild(row)})}
function saveInspectionSession(){if(!inspectionDraftItems.length)return alert('請先加入至少一個抽盤品項。');ensureInspectionState();const session={id:uid('inspection'),createdAt:new Date().toISOString(),auditor:$('inspectionAuditor')?.value.trim()||state.branding?.managerName||'管理者',note:$('inspectionSessionNote')?.value.trim()||'',items:inspectionDraftItems.map(x=>({...x}))};state.inventoryInspections.unshift(session);inspectionDraftItems=[];$('inspectionSessionNote').value='';save();renderInspection();alert(`已儲存本次抽盤，共 ${session.items.length} 項，異常 ${session.items.filter(x=>x.difference!==0).length} 項。`)}
function renderInspectionSummary(){ensureInspectionState();const now=new Date(),start=new Date(now);start.setDate(now.getDate()-((now.getDay()+6)%7));start.setHours(0,0,0,0);const week=state.inventoryInspections.filter(x=>new Date(x.createdAt)>=start),latest=state.inventoryInspections[0],totalItems=week.reduce((s,x)=>s+x.items.length,0),abnormal=week.reduce((s,x)=>s+x.items.filter(i=>i.difference!==0).length,0);const el=$('inspectionSummary');if(el)el.innerHTML=`<div class="summary-card"><small>本週抽盤</small><strong>${week.length} 次</strong></div><div class="summary-card"><small>本週品項</small><strong>${totalItems} 項</strong></div><div class="summary-card"><small>本週異常</small><strong class="${abnormal?'negative':'positive'}">${abnormal} 項</strong></div><div class="summary-card"><small>最近一次</small><strong>${latest?new Date(latest.createdAt).toLocaleDateString('zh-TW'):'尚無'}</strong></div>`}
function historySessionCard(session,type='random'){const abnormal=session.items.filter(x=>x.difference!==0).length,details=document.createElement('details');details.className='inspection-history-card';details.innerHTML=`<summary><div><strong>${new Date(session.createdAt).toLocaleString('zh-TW')}</strong><small>${esc(session.auditor||'')} ${session.note?`｜${esc(session.note)}`:''}</small></div><div class="history-count"><span>${session.items.length} 項</span><b class="${abnormal?'negative':'positive'}">異常 ${abnormal}</b></div></summary><div class="history-items">${session.items.map(x=>`<div class="history-item"><div><strong>${esc(x.name||x.sku||x.code)}</strong><small>${esc(x.sku||x.code)}</small></div><span>系統 ${fmt(x.systemQty)}／實際 ${fmt(x.actualQty)}</span>${differenceBadge(x.difference)}</div>`).join('')}</div>`;return details}
function renderInspectionHistory(){const el=$('inspectionHistory');if(!el)return;el.innerHTML='';if(!state.inventoryInspections.length){el.innerHTML='<div class="empty-state">尚無抽盤紀錄。</div>';return}state.inventoryInspections.forEach(x=>el.appendChild(historySessionCard(x)))}
function saveDisplayItem(){const code=normalizeBarcode($('displayBarcode')?.value),p=productFromCode(code),name=$('displayProductName')?.value.trim()||p?.name||'',expectedQty=num($('displayExpectedQty')?.value),status=$('displayStatus')?.value||'展示中',note=$('displayNote')?.value.trim()||'';if(!code)return alert('請輸入或掃描料號／EAN。');if(!name)return alert('請輸入品名。');if(expectedQty<0)return alert('展示數量不可小於 0。');ensureInspectionState();const existing=state.displayCatalog.find(x=>x.code===code||x.sku===code||x.ean===code);const item={id:existing?.id||uid('display'),code,sku:p?.sku||existing?.sku||code,ean:/^\d{13}$/.test(code)?code:(existing?.ean||''),name,expectedQty,status,note,lastCountedAt:existing?.lastCountedAt||'',updatedAt:new Date().toISOString()};if(existing)Object.assign(existing,item);else state.displayCatalog.push(item);rememberProduct(code,name,item.sku);['displayBarcode','displayProductName','displayNote'].forEach(id=>{$(id).value=''});$('displayExpectedQty').value='1';save();renderDisplayCatalog()}
function editDisplayItem(id){const x=state.displayCatalog.find(i=>i.id===id);if(!x)return;$('displayBarcode').value=x.code||x.sku;$('displayProductName').value=x.name;$('displayExpectedQty').value=x.expectedQty;$('displayStatus').value=x.status;$('displayNote').value=x.note||'';$('displayBarcode').scrollIntoView({behavior:'smooth',block:'center'})}
function deleteDisplayItem(id){const x=state.displayCatalog.find(i=>i.id===id);if(!x||!confirm(`確定刪除「${x.name}」？`))return;state.displayCatalog=state.displayCatalog.filter(i=>i.id!==id);save();renderDisplayCatalog()}
function renderDisplayCatalog(){ensureInspectionState();const el=$('displayCatalogList');if(!el)return;el.innerHTML='';if(!state.displayCatalog.length){el.innerHTML='<div class="empty-state">尚未建立展示品清冊。</div>';return}state.displayCatalog.slice().sort((a,b)=>a.status.localeCompare(b.status)||a.name.localeCompare(b.name)).forEach(x=>{const row=document.createElement('article');row.className='display-catalog-item';row.innerHTML=`<div><strong>${esc(x.name)}</strong><small>${esc(x.sku||x.code)}${x.ean?`｜EAN ${esc(x.ean)}`:''}</small><span>展示數量 ${fmt(x.expectedQty)}｜${esc(x.status)}</span><p>${x.lastCountedAt?`最後盤點 ${new Date(x.lastCountedAt).toLocaleString('zh-TW')}`:'尚未盤點'}${x.note?`｜${esc(x.note)}`:''}</p></div><div class="display-item-actions"><button type="button" class="btn btn-soft edit">編輯</button><button type="button" class="btn btn-danger delete">刪除</button></div>`;row.querySelector('.edit').addEventListener('click',()=>editDisplayItem(x.id));row.querySelector('.delete').addEventListener('click',()=>deleteDisplayItem(x.id));el.appendChild(row)})}
function startDisplayAudit(){ensureInspectionState();const active=state.displayCatalog.filter(x=>x.status==='展示中');if(!active.length)return alert('目前沒有「展示中」的品項。');displayAuditDraft={id:uid('display-audit'),createdAt:new Date().toISOString(),auditor:state.branding?.managerName||'管理者',items:active.map(x=>({catalogId:x.id,code:x.code,sku:x.sku,name:x.name,systemQty:x.expectedQty,actualQty:x.expectedQty,difference:0,checked:false,note:''}))};$('displayAuditCard')?.classList.remove('hidden');renderDisplayAuditDraft();$('displayAuditCard')?.scrollIntoView({behavior:'smooth',block:'start'})}
function renderDisplayAuditDraft(){const el=$('displayAuditList');if(!el||!displayAuditDraft)return;const checked=displayAuditDraft.items.filter(x=>x.checked).length;$('displayAuditProgress').textContent=`已確認 ${checked} / ${displayAuditDraft.items.length}`;$('displayAuditTime').textContent=new Date(displayAuditDraft.createdAt).toLocaleString('zh-TW');el.innerHTML='';displayAuditDraft.items.forEach((x,i)=>{const row=document.createElement('article');row.className=`display-audit-item ${x.checked?'checked':''}`;row.innerHTML=`<label class="audit-check"><input type="checkbox" ${x.checked?'checked':''}><span></span></label><div class="audit-product"><strong>${esc(x.name)}</strong><small>${esc(x.sku||x.code)}</small></div><label>清冊<input type="number" value="${x.systemQty}" disabled></label><label>實際<input class="audit-actual" type="number" inputmode="numeric" min="0" step="1" value="${x.actualQty}"></label><div class="audit-diff">${differenceBadge(x.difference)}</div>`;row.querySelector('input[type=checkbox]').addEventListener('change',e=>{x.checked=e.target.checked;renderDisplayAuditDraft()});row.querySelector('.audit-actual').addEventListener('input',e=>{x.actualQty=num(e.target.value);x.difference=inspectionDifference(x.systemQty,x.actualQty);x.checked=true;renderDisplayAuditDraft()});el.appendChild(row)})}
function saveDisplayAudit(){if(!displayAuditDraft)return;const unchecked=displayAuditDraft.items.filter(x=>!x.checked);if(unchecked.length&&!confirm(`尚有 ${unchecked.length} 項未確認，仍要完成嗎？`))return;state.displayAudits.unshift(displayAuditDraft);const t=displayAuditDraft.createdAt;displayAuditDraft.items.forEach(i=>{const c=state.displayCatalog.find(x=>x.id===i.catalogId);if(c)c.lastCountedAt=t});const abnormal=displayAuditDraft.items.filter(x=>x.difference!==0).length;displayAuditDraft=null;$('displayAuditCard')?.classList.add('hidden');save();renderInspection();alert(`展示抽盤已完成，共 ${state.displayAudits[0].items.length} 項，異常 ${abnormal} 項。`)}
function renderDisplaySummary(){ensureInspectionState();const active=state.displayCatalog.filter(x=>x.status==='展示中'),latest=state.displayAudits[0],abnormal=latest?latest.items.filter(x=>x.difference!==0).length:0,el=$('displaySummary');if(el)el.innerHTML=`<div class="summary-card"><small>展示中品項</small><strong>${active.length} 項</strong></div><div class="summary-card"><small>清冊展示數</small><strong>${active.reduce((s,x)=>s+num(x.expectedQty),0)}</strong></div><div class="summary-card"><small>最近異常</small><strong class="${abnormal?'negative':'positive'}">${abnormal} 項</strong></div><div class="summary-card"><small>最後盤點</small><strong>${latest?new Date(latest.createdAt).toLocaleDateString('zh-TW'):'尚無'}</strong></div>`}
function renderDisplayAuditHistory(){const el=$('displayAuditHistory');if(!el)return;el.innerHTML='';if(!state.displayAudits.length){el.innerHTML='<div class="empty-state">尚無展示抽盤紀錄。</div>';return}state.displayAudits.forEach(x=>el.appendChild(historySessionCard(x,'display')))}
function renderInspection(){ensureInspectionState();if(!$('page-inspection'))return;$('inspectionDraftTime').textContent=new Date().toLocaleString('zh-TW');if($('inspectionAuditor')&&!$('inspectionAuditor').value)$('inspectionAuditor').value=state.branding?.managerName||'管理者';renderInspectionSummary();renderInspectionDraft();renderInspectionHistory();renderDisplaySummary();renderDisplayCatalog();renderDisplayAuditHistory();if(displayAuditDraft)renderDisplayAuditDraft()}
async function openScanner(target){scannerTarget=target;const modal=$('scannerModal'),msg=$('scannerMessage');if(!modal)return;modal.classList.remove('hidden');msg.textContent='正在啟動相機…';if(typeof Html5Qrcode==='undefined'){msg.textContent='掃描元件載入失敗，請改用手動輸入。';return}try{scannerInstance=new Html5Qrcode('barcodeReader');await scannerInstance.start({facingMode:'environment'},{fps:10,qrbox:{width:280,height:150},formatsToSupport:[Html5QrcodeSupportedFormats.EAN_13,Html5QrcodeSupportedFormats.CODE_128,Html5QrcodeSupportedFormats.CODE_39,Html5QrcodeSupportedFormats.QR_CODE]},code=>handleScanResult(code),()=>{});msg.textContent='請將條碼置於框線中央。'}catch(e){console.error(e);msg.textContent='無法開啟相機，請確認已允許相機權限，或改用手動輸入。'}}
async function closeScanner(){try{if(scannerInstance){await scannerInstance.stop();await scannerInstance.clear()}}catch(e){}scannerInstance=null;$('scannerModal')?.classList.add('hidden');const reader=$('barcodeReader');if(reader)reader.innerHTML=''}
async function handleScanResult(raw){const code=normalizeBarcode(raw);await closeScanner();if(scannerTarget==='inspection'){const input=$('inspectionBarcode'),name=$('inspectionProductName');input.value=code;const p=productFromCode(code);if(p)name.value=p.name||'';input.dispatchEvent(new Event('change'))}else if(scannerTarget==='display'){const input=$('displayBarcode'),name=$('displayProductName');input.value=code;const p=productFromCode(code);if(p)name.value=p.name||'';input.dispatchEvent(new Event('change'))}else if(scannerTarget==='displayAudit'&&displayAuditDraft){const item=displayAuditDraft.items.find(x=>[x.code,x.sku,x.ean].map(normalizeBarcode).includes(code));if(!item){alert('此條碼不在目前展示清冊中。');return}item.checked=true;renderDisplayAuditDraft();requestAnimationFrame(()=>{const idx=displayAuditDraft.items.indexOf(item);$('displayAuditList')?.children[idx]?.scrollIntoView({behavior:'smooth',block:'center'})})}}
function bindInspection(){document.querySelectorAll('.inspection-tab').forEach(x=>x.addEventListener('click',()=>switchInspectionTab(x.dataset.inspectionTab)));$('addInspectionItemBtn')?.addEventListener('click',addInspectionItem);$('saveInspectionSessionBtn')?.addEventListener('click',saveInspectionSession);$('clearInspectionDraftBtn')?.addEventListener('click',()=>{if(inspectionDraftItems.length&&!confirm('確定清空本次尚未儲存的內容？'))return;inspectionDraftItems=[];renderInspectionDraft()});$('saveDisplayItemBtn')?.addEventListener('click',saveDisplayItem);$('startDisplayAuditBtn')?.addEventListener('click',startDisplayAudit);$('saveDisplayAuditBtn')?.addEventListener('click',saveDisplayAudit);$('cancelDisplayAuditBtn')?.addEventListener('click',()=>{displayAuditDraft=null;$('displayAuditCard')?.classList.add('hidden')});document.querySelectorAll('[data-scan-target]').forEach(x=>x.addEventListener('click',()=>openScanner(x.dataset.scanTarget)));$('closeScannerBtn')?.addEventListener('click',closeScanner);$('scannerManualBtn')?.addEventListener('click',closeScanner);bindProductLookup('inspectionBarcode','inspectionProductName');bindProductLookup('displayBarcode','displayProductName')}
